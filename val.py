...
def main():
    ...
    with torch.no_grad():
        for input, target, meta in tqdm(val_loader, total=len(val_loader)):
            ...
            iou, dice, hd, hd95, recall, specificity, precision, acc, mae, auc = indicators(output, target)
            ...
            acc_.update(acc, input.size(0))
            mae_.update(mae, input.size(0))
            auc_.update(auc, input.size(0))
    ...
    print('acc: %.4f' % acc_.avg)
    print('mae: %.4f' % mae_.avg)
    print('auc: %.4f' % auc_.avg)
